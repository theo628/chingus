...
import {
  ...
  Smartphone, ShieldCheck, LogOut, Lock, GraduationCap, Volume2, Flame, Trophy, RefreshCw, ChevronDown, Languages
} from "lucide-react";
...
/* ─────────────────────────  App shell  ───────────────────────── */
function App({ initialLaunch }) {
  const [loaded, setLoaded] = useState(false);
  const [me, setMe] = useState(null);
  const [editing, setEditing] = useState(false);
  const [started, setStarted] = useState(!!initialLaunch);
...
function FaqItem({ q, a }) {
  ...
  return (
    <div className="rounded-2xl" style={{ backgroundColor: C.card, border: `1.5px solid ${C.line}` }}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between text-left px-4 py-3.5">
        <span className="text-sm font-semibold" style={{ color: C.ink }}>{q}</span>
        <ChevronDown size={16} style={{ color: C.sub, transform: open ? "rotate(180deg)" : "none", transition: "transform .15s" }} />
      </button>
...
export default function Site() {
  ...
  if (view === "app") {
    return (
      <div>
        <div className="fixed top-0 left-0 right-0" style={{ zIndex: 50, backgroundColor: C.ink }}>
          <div className="max-w-md mx-auto px-4 py-1.5 flex items-center justify-between">
            <button onClick={() => setView("site")} className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: "#CFC7E8" }}>
  ...
  return (
    <div className="min-h-screen kc-body" style={{ backgroundColor: C.bg }}>
      ...
      {/* Nav */}
      <header className="sticky top-0" style={{ zIndex: 40, backgroundColor: C.bg, borderBottom: `1.5px solid ${C.line}` }}>
        <div className="max-w-3xl mx-auto px-5 py-3 flex items-center justify-between">
          ...
          <button onClick={() => setView("app")} className="rounded-full px-4 py-2 text-sm font-semibold" style={{ backgroundColor: C.pink, color: "#fff" }}>
            Open the app
          </button>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-5">
        ...
        <section className="pt-14 pb-12 text-center">
          <div className="kc-display leading-none" style={{ fontSize: "clamp(56px, 12vw, 96px)", color: C.ink }}>
      ...
      <footer style={{ borderTop: `1.5px solid ${C.line}` }}>
        <div className="max-w-3xl mx-auto px-5 py-6 flex items-center justify-between text-xs" style={{ color: C.sub }}>
